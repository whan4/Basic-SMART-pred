...
server <- function(input, output, session) {
  ...
  observeEvent(input$runButton, {
    ...
    output$val_performance_metrics <- renderTable({
      # Test table size
      # results <- readRDS("~/Desktop/ML_pipeline/data/pca_results.rds")
      # results$decision_tree <- NULL
      # results$graident_boosting <- NULL
      # results$adaboost <- NULL
    ...
    # target <- c("class")
      ...
      metrics <- lapply(names(rv$results), function(model_name) {
        ...
        if (input$task_type == "Classification") {
          ...
          # Make sure the factor levels of the predictions and the actual values are consistent
          predictions$pred <- factor(predictions$pred, levels = levels(predictions$obs))
          conf_mat <- confusionMatrix(predictions$pred, predictions$obs)
          accuracy_ci_lower <- conf_mat$overall["AccuracyLower"]
          accuracy_ci_upper <- conf_mat$overall["AccuracyUpper"]
    ...
    output$roc_curves <- renderPlot({
      ...
      roc_curves <- lapply(names(rv$results), function(model_name) {
        model <- rv$results[[model_name]]
      ...
      legend_text <- sapply(1:length(roc_curves), function(i) {
        sprintf("%s (AUC: %.2f)", names(rv$results)[i], auc(roc_curves[[i]]))
      })
    ...
    #Test figure size
    #results <- readRDS("~/Desktop/ML_pipeline/data/varimp_results.rds")
    #results$decision_tree <- NULL
    #results$graident_boosting <- NULL
    #results$adaboost <- NULL
    #results$xgboost <- NULL
